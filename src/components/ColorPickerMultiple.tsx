...
interface ColorPickerMultipleProps {
  color: string;
...
const ColorPickerMultiple: React.FC<ColorPickerMultipleProps> = ({
  color,
  onChange,
  onRemove,
  canRemove,
    ...
    <div className={cn('space-y-2', className)}>
      <div className="flex items-center space-x-4">
        <div className="flex-1">
          <Label htmlFor={`color-picker-${index}`} className="text-sm font-medium">
            {index === 0 ? 'Primary Brand Color' : `Brand Color ${index + 1}`}
          </Label>
          <div className="mt-1.5 flex items-center gap-2">
            <div 
              className="w-10 h-10 rounded border overflow-hidden shadow-sm"
